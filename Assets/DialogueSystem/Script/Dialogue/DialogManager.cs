...
public class DialogManager : MonoBehaviour
{
    ...
    /*
        IEnumerator Dialogue_Coroutine(float delay_, Dialogue dialog)
        {
            is_start = true;
            bool HasStyle = false;
            string dialogue_test = dialog.text;
            ...
            if (current_branch.Equals(dialog.branch_id))
            {
                ...
                int lenghtLine = dialogue_test.Length;
                string tmp = "";
                var delayy = delay_;
                isClicked = false;
                for (int i = 0; i < lenghtLine; i++)
                {
                    ...
                    if (dialog.isLeft || !IsItsymmetric)
                    {
                        changeImage(img_speaker_1, dialog, i);
                        string char_temp = dialogue_test[i].ToString();
                        if (HasStyle)
                        {
                        ...
                        text_speaker_1_txt.text = tmp;



                        // text_speaker_1_txt.SetText(char_temp,false);
                        if (IsItsymmetric)
                        {
                            text_speaker_2_txt.gameObject.SetActive(false);
                            name_speaker_2_txt.text = "";
                            text_speaker_2_txt.text = "";
                        }
                        //   Debug.Log(delayy);
                    ...
                    {
                        changeImage(img_speaker_2, dialog, i);
                        text_speaker_1_txt.gameObject.SetActive(false);
                        text_speaker_2_txt.gameObject.SetActive(true);
                        tmp = tmp + dialogue_test[i];
                        name_speaker_1_txt.text = "";
                        text_speaker_1_txt.text = "";
                        name_speaker_2_txt.text = dialog.speaker;
                        text_speaker_2_txt.text = tmp;
                        yield return new WaitForSeconds(delayy);
                    }
                }
                delayy = delay_;
                ...
                isMultipleChoice = Check_Choice(dialog.multiple_choice);
